        ...
        return(
            <div key={t.id} className="col-3 pb-5">
                <div className="box pb-5 position-relative">
                    ...
                    <div className="text card bg-white position-absolute start-50 translate-middle-x px-2 py-3">
                        <p className="fw-medium fs-5">{t.name}</p>
                        <span className="grey mb-3">{t.title}</span>
                        <div className="icons d-flex align-items-center justify-content-center gap-3">
                            {t.links.map(link=>{
                                return(
                                    <Link to={link.link} key={Date.now().toString(36) + Math.random().toString(36).substr(2)} className="icon border border-1 rounded-circle p-3" role="button">
